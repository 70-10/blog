...
| [0006](../../../../../adr/0006-build-our-own-admin-screen-for-writing.md)                       | 記事を書く画面を自分で作り、公開までその中で完結させる                       | この Unit が画面の土台・認証・読み書きの裏側を作る                                                                               |
| [0008](../../../../../adr/0008-admin-runs-as-its-own-pages-project-behind-cloudflare-access.md) | 管理画面は独立した Pages プロジェクトに置き、入口は Cloudflare Access で守る | 0006 が白紙にしていた技術構成を埋めた。**この Unit の Stage 1 で中断して決めた**（[open-questions.md](open-questions.md) の Q2） |
...
[ADR 0008](../../../../../adr/0008-admin-runs-as-its-own-pages-project-behind-cloudflare-access.md) で管理画面を別の Pages プロジェクトにしたので、次はこのリポジトリの中に新しく作る。既存の実装を変えるのではなく足す側。

| 作るもの                     | 置き場の候補                                                                                     | 決めるのは |
| ---------------------------- | ------------------------------------------------------------------------------------------------ | ---------- |
| 管理画面のパッケージ         | `packages/` か `tools/`（[constraints.md](constraints.md) の「新しいワークスペースパッケージ」） | Stage 2    |
| Access の JWT を検証する処理 | 同パッケージ内                                                                                   | Stage 2    |
| リポジトリを読み書きする処理 | 同パッケージ内（GitHub Contents API）                                                            | Stage 2    |

## 未解決

残るのは下書きの保存の形（Q1）とテストの実行環境（Q3）で、どちらも Construction の後続ステージで決まる。人の判断を待つものは無い。詳細は [open-questions.md](open-questions.md) にある。
